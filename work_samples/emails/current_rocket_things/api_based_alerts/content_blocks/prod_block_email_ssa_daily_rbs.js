...
var user = getRocketHomesData("GET", url, apiKey, token, "Daily SSA", "Email", "RHD_API_Errors");
validateSsaApiResponse(user, url, "Daily SSA", "Email", "RHD_API_Errors");
...
/* SINGLE STATIC LINKS */
Variable.SetValue("@notifications_url", Variable.GetValue("@environment") + "/notifications?openInApp=true&utm_source=rockethomes&utm_medium=email&utm_campaign=rh-ssa&utm_term=saved-search-alerts&utm_content=rhdc-ssa-daily&utm_cta=manage-notifications");
Variable.SetValue("@alert_area", user.saved_searches.items[0]._links.saved_search_web_url.href);
Variable.SetValue("@alert_area_url", Variable.GetValue("@environment") + Variable.GetValue("@alert_area") + "?openInApp=true&utm_source=rockethomes&utm_medium=email&utm_campaign=rh-ssa&utm_term=saved-search-alerts&utm_content=rhdc-ssa-daily&utm_cta=view-all-homes");
...
/* ESTABLISH SEARCH TOTAL */
Variable.SetValue("@search_total", user.saved_searches.items[0].matches._meta.total);
...
if (Variable.GetValue("@search_total") >= 3) {
    ...
    var limiter = Variable.GetValue("@search_total")
}

for (var i = 0; i < limiter; i++) {

    Variable.SetValue("@show_0" + i, "yes")

    /* SET PER MATCH VARIABLES */
    var item_number = i
    var listing = user.saved_searches.items[0].matches.items[item_number].listing

    /* LISTING INFO */
    Variable.SetValue("@listing_web_url_href_0" + i, user.saved_searches.items[0].matches.items[i].listing._links.listing_web_url.href);
    Variable.SetValue("@address_0" + i, user.saved_searches.items[0].matches.items[i].listing.address);
    Variable.SetValue("@city_0" + i, user.saved_searches.items[0].matches.items[i].listing.city);
    Variable.SetValue("@state_code_0" + i, user.saved_searches.items[0].matches.items[i].listing.state_code);
    Variable.SetValue("@zip_code_0" + i, user.saved_searches.items[0].matches.items[i].listing.zip_code);
    Variable.SetValue("@property_type_0" + i, user.saved_searches.items[0].matches.items[i].listing.property_type);
    Variable.SetValue("@square_feet_0" + i, Number(user.saved_searches.items[0].matches.items[i].listing.square_feet));
    Variable.SetValue("@square_feet_range_0" + i, user.saved_searches.items[0].matches.items[i].listing.square_feet_range);
    Variable.SetValue("@lot_size_acres_0" + i, user.saved_searches.items[0].matches.items[i].listing.lot_size_acres);
    Variable.SetValue("@list_price_0" + i, Number(user.saved_searches.items[0].matches.items[i].listing.list_price));
    Variable.SetValue("@mls_number_0" + i, user.saved_searches.items[0].matches.items[i].listing.mls_number);
    Variable.SetValue("@bedrooms_total_0" + i, user.saved_searches.items[0].matches.items[i].listing.bedrooms_total);
    Variable.SetValue("@bathrooms_total_0" + i, user.saved_searches.items[0].matches.items[i].listing.bathrooms_total);
    Variable.SetValue("@bathrooms_full_0" + i, user.saved_searches.items[0].matches.items[i].listing.bathrooms_full);
    Variable.SetValue("@bathrooms_partial_0" + i, user.saved_searches.items[0].matches.items[i].listing.bathrooms_partial);
    Variable.SetValue("@main_photo_0" + i, user.saved_searches.items[0].matches.items[i].listing.photos_sample[0].url_640);
    Variable.SetValue("@sub_photo_01_0" + i, user.saved_searches.items[0].matches.items[i].listing.photos.items[1].url_640);
    Variable.SetValue("@sub_photo_02_0" + i, user.saved_searches.items[0].matches.items[i].listing.photos.items[2].url_640);
    Variable.SetValue("@sub_photo_03_0" + i, user.saved_searches.items[0].matches.items[i].listing.photos.items[3].url_640);
    Variable.SetValue("@list_office_name_0" + i, user.saved_searches.items[0].matches.items[i].listing.list_office_name);
    Variable.SetValue("@idx_logo_0" + i, user.saved_searches.items[0].matches.items[i].listing.compliance.idx_logo);
    Variable.SetValue("@disclaimer_0" + i, user.saved_searches.items[0].matches.items[i].listing.compliance.disclaimer_html);
    Variable.SetValue("@public_remarks_0" + i, user.saved_searches.items[0].matches.items[i].listing.public_remarks);

    /* COMPLIANCE INFO */
    Variable.SetValue("@show_mls_number_0" + i, user.saved_searches.items[0].matches.items[i].listing.compliance.show_mls_number);
    Variable.SetValue("@hide_badges_0" + i, user.saved_searches.items[0].matches.items[i].listing.compliance.hide_badges);
    Variable.SetValue("@hide_sqft_0" + i, user.saved_searches.items[0].matches.items[i].listing.compliance.hide_sqft);
    Variable.SetValue("@show_sqft_range_0" + i, user.saved_searches.items[0].matches.items[i].listing.compliance.show_sqft_range);
    Variable.SetValue("@show_primary_photo_only_0" + i, user.saved_searches.items[0].matches.items[i].listing.compliance.show_primary_photo_only);
    Variable.SetValue("@virtual_tours_0" + i, user.saved_searches.items[0].matches.items[i].listing.virtual_tours[0].url);

    /* DYNAMIC PROPERTY LINKS */
    Variable.SetValue("@listing_web_url_0" + i, user.saved_searches.items[0].matches.items[i].listing._links.listing_web_url.href);
    ...
    /* BUY SMART INFO */
    Variable.SetValue("@is_eligible_for_buy_smart_national_0" + i, user.saved_searches.items[0].matches.items[i].listing.is_eligible_for_buy_smart_national);
    Variable.SetValue("@buy_smart_discount_0" + i, user.saved_searches.items[0].matches.items[i].listing.buy_smart_discount);
    Variable.SetValue("@buy_smart_discount_percentage_0" + i, user.saved_searches.items[0].matches.items[i].listing.buy_smart_discount_percentage);

    /* SELL SMART INFO */
    Variable.SetValue("@is_eligible_for_sell_smart_national_0" + i, user.saved_searches.items[0].matches.items[i].listing.is_eligible_for_sell_smart_national);
    Variable.SetValue("@sell_smart_discount_0" + i, user.saved_searches.items[0].matches.items[i].listing.sell_smart_discount);
    Variable.SetValue("@sell_smart_discount_percentage_0" + i, user.saved_searches.items[0].matches.items[i].listing.sell_smart_discount_percentage);
...
if (Variable.GetValue("@show_mls_number_0" + i) == true) {
...
Variable.SetValue("@square_feet_0" + i, Variable.GetValue("@square_feet_range_0" + i).toString());
}
...
SET @header_greeting = CONCAT("Hi ", @client_first_name, ", we found a <strong>new match</strong> for you in ", @city_00, ", ", @state_code_00, ".")

IF (@search_total > 1) THEN
SET @header_greeting = CONCAT("Hi ", @client_first_name, ", we found <strong>new matches </strong> for you in ", @city_00, ", ", @state_code_00, ".")
ENDIF
